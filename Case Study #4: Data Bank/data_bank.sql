...
SELECT region_name, COUNT(node_id) AS num_node
FROM customer_nodes AS cn
LEFT JOIN regions AS r
ON cn.region_id = r.region_id
GROUP BY region_name;
--- 3. How many customers are allocated to each region?
SELECT region_name, COUNT(DISTINCT(customer_id)) AS num_cus
FROM customer_nodes AS cn
LEFT JOIN regions AS r
ON cn.region_id = r.region_id
GROUP BY region_name;
--- 4. How many days on average are customers reallocated to a different node?
SELECT ROUND(AVG(end_date - start_date), 2) AS avg_day
FROM customer_nodes
WHERE end_date != '9999-12-31';
--- 5. What is the median, 80th and 95th percentile for this same reallocation days metric for each region?
WITH day_metric AS (
	SELECT region_name,
		(end_date::DATE - start_date::DATE) AS reallocate_day
	FROM customer_nodes AS cn
	LEFT JOIN regions AS r
	ON cn.region_id = r.region_id
	WHERE end_date != '9999-12-31'
...
SELECT region_name,
		(SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY reallocate_day) FROM day_metric AS d WHERE d.region_name = day_metric.region_name) AS median,
		(SELECT PERCENTILE_CONT(0.8) WITHIN GROUP (ORDER BY reallocate_day) FROM day_metric AS d WHERE d.region_name = day_metric.region_name) AS per_80,
		(SELECT PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY reallocate_day) FROM day_metric AS d WHERE d.region_name = day_metric.region_name) AS per_95
FROM day_metric
GROUP BY region_name;
/*
...
SELECT txn_type, COUNT(txn_type) AS num_each_type, SUM(txn_amount) AS total_amount
...
WITH monthly_count AS (
	SELECT customer_id, DATE_PART('month', txn_date::DATE) AS month,
			COUNT(CASE
			 	WHEN txn_type = 'deposit' THEN 1
...
FROM monthly_count
WHERE deposit > 1
AND (purchase > 0 OR withdrawal > 0)
GROUP BY 1;
...
WITH remain AS (
	SELECT customer_id, DATE_PART('month', txn_date::DATE) AS month,
		SUM(CASE 
			WHEN txn_type = 'deposit' THEN txn_amount
			ELSE -txn_amount END) AS remain
	FROM customer_transactions
	GROUP BY 1, 2
	ORDER BY 1
)
SELECT customer_id, month,
		SUM(remain) OVER(PARTITION BY customer_id
...
WITH remain AS (
	SELECT customer_id, 
			DATE_PART('month', txn_date::DATE) AS month,
			SUM(CASE 
			   		WHEN txn_type = 'deposit' THEN txn_amount
			   		ELSE - txn_amount END) AS remain
	FROM customer_transactions
	...
	closing_balance AS (
		SELECT customer_id,
				month,
				SUM(remain) OVER(PARTITION BY customer_id, month
								ORDER BY month ROWS BETWEEN
								UNBOUNDED PRECEDING AND CURRENT ROW) AS closing_balance
		FROM remain
		GROUP BY 1, 2, remain
	...
	increase AS(
		SELECT customer_id,
				month,
				closing_balance,
				(100 * (closing_balance - LAG(closing_balance, 1) OVER(PARTITION BY customer_id 
																	  ORDER BY month)) / NULLIF(LAG(closing_balance, 1) OVER(PARTITION BY customer_id 
																															 ORDER BY month), 0)) AS pct_increase
		FROM closing_balance
)
SELECT (100 * COUNT(DISTINCT(customer_id)) / (SELECT COUNT(DISTINCT(customer_id)) FROM customer_transactions)) AS pct_cus
FROM increase
...
balance AS (
	SELECT *,
			SUM(amount) OVER(PARTITION BY customer_id, month
							ORDER BY month ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS balance
	FROM amount
),
monthly_allocation AS(
	SELECT *,
			LAG(balance, 1, 0) OVER(PARTITION BY customer_id
								   ORDER BY customer_id, month) AS monthly_allocation
	FROM balance
)
/* 
Customer segmentation base on last month account balance
(Customer with a negative previous month balance will receive allocation = 0)
*/
SELECT month, 
		SUM(CASE WHEN monthly_allocation < 0 THEN 0
		   		ELSE monthly_allocation END) AS total_allocation
